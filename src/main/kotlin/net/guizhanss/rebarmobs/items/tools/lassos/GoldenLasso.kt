...
class GoldenLasso(item: ItemStack) : BaseMobLasso(item) {
    override val holdingDuration: Int
        get() = HOLDING_DURATION

    override fun capture(entity: LivingEntity): CaptureResult = captureByTag(entity, RebarMobsTag.GOLDEN_LASSO_ALLOWED)

    companion object {
        private val settings = ConfigSection.fromSettings(RebarMobsKeys.GOLDEN_LASSO)
